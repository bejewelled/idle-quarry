    ...
    <div class="main-page grid grid-cols-12">
        <div class="px-2 res-display col-span-3">
            ...
            text-left text-small pb-1"
                ...
                <div
                    class="alog-minimize col-span-3 game-btn text-center game-text"
                    on:click={() => toggleAlog()}
                >
                    {alogShow ? "-" : "+"}
                </div>
                ...
                {#if alogShow}
                    <div
                        class="alog-cont col-span-12 h-64 relative bg-gray-700 border-4 border-gray-600"
                    >
                        <div
                            class="scrollable-area absolute inset-0 overflow-y-auto"
                        >
                            {#each $activityLog.slice().reverse() as a}
                                <div
                                    class="alog-text text-small {a[1]} col-span-12 pt-1 pl-1"
                                >
                                    {a[0]}
                                </div>
                            {/each}
        ...
        <div class="px-2 py-2 main-panel col-span-8 flexbox">
            <div class="py-1 control-buttons resize-none">
                ...
                >
                <button
                    class="py-1 text-small save-btn control-btn"
                    on:click={() => load(true)}>Import</button
                >
                <button
                    class="py-1 text-small save-btn control-btn {exportConfirm
                        ? 'bg-green-400'
                        : ''}"
                    on:click={() => save(true)}>Export</button
                >
                <button
                    class="py-1 px-1 text-small save-btn control-btn"
                    on:click={() => cycleBuyAmount()}>Buy x{buyAmount}</button
                >
                <button
                    class="py-1 px-1 text-small save-btn has-tooltip {$settings[
                        'maxBuy'
                    ]
                        ? 'game-btn-toggleon'
                        : 'control-btn'}"
                ...
                <button
                    class="py-1 px-1 text-small save-btn control-btn"
                    on:click={() => changeTab("help")}>Help!</button
                >
                <button
                    class="py-1 px-1 text-small save-btn control-btn"
                ...
                <button class="text-xs text-gray-600">v0.0.5a-1 (TESTING)</button>
                <!-- mining level bar -->
            ...
            <div class="row-span-1 tab-buttons">
                {#key clockTabs}
                    {#each ref.tabs as t}
                        <button
                            class="px-1 py-1 text-small
                    {!tabUnlockCriteria[t]
                                ? 'control-btn-nounlock'
                                : tabUnlockCriteria[t]()
                                ? t === tab
            ...
            <div class="row-span-10 main-panel-display">
                ...
                {#if (tabUnlockCriteria[tab] && !tabUnlockCriteria[tab]()) || !tabUnlockCriteria[tab]}
                    <div class="game-text">
                        {ref.tabNotUnlockedText[tab] || ""}
                    </div>
                {/if}
            </div>
        </div>
...
<script lang='ts'>
    ...
    const tabUnlockCriteria = {
        ...
        keys: () => true,
        beacons: () =>
            $wallet["beacons"] || formula.sumArray($beaconActivations) > 0,
    ...
    const save = async (isExport = false) => {
        ...
        localStorage.setItem(
            "miningUpgradeLevelsBought",
            JSON.stringify($miningUpgradeLevelsBought)
        );
        localStorage.setItem(
            "miningUpgradeLevelsFree",
        ...
        localStorage.setItem(
            "miningUpgradeLevelsTemp",
            JSON.stringify($miningUpgradeLevelsTemp)
        );
        localStorage.setItem(
            "miningUpgradeLevelsBoughtTemp",
            JSON.stringify($miningUpgradeLevelsBoughtTemp)
        );
        localStorage.setItem(
            "miningUpgradeLevelsFreeTemp",
            JSON.stringify($miningUpgradeLevelsFreeTemp)
        );
        ...
        localStorage.setItem(
            "beaconUpgradeLevels",
            JSON.stringify($beaconUpgradeLevels)
        );
        localStorage.setItem("resources", JSON.stringify($resources));
        localStorage.setItem(
            "beaconActivations",
            JSON.stringify($beaconActivations)
        );
        ...
        localStorage.setItem(
            "keyUpgradeLevels",
            JSON.stringify($keyUpgradeLevels)
        );
        ...
        localStorage.setItem(
            "keyCraftMastery",
            JSON.stringify($keyCraftMastery)
        );
        localStorage.setItem("startOfGame", JSON.stringify($startOfGame));
        localStorage.setItem("settings", JSON.stringify($settings));
        localStorage.setItem(
            "antiFlickerFlags",
            JSON.stringify($antiFlickerFlags)
        );
        localStorage.setItem("saveVersion", JSON.stringify($saveVersion));
        localStorage.setItem(
            "automationItemsUnlocked",
            JSON.stringify($automationItemsUnlocked)
        );
        localStorage.setItem(
            "activityLogShow",
            JSON.stringify($activityLogShow)
        );
        localStorage.setItem(
            "challengeActive",
            JSON.stringify($challengeActive)
        );
        localStorage.setItem(
            "challengesCompleted",
        ...
        localStorage.setItem(
            "beaconSmartSplits",
            JSON.stringify($beaconSmartSplits)
        );
        localStorage.setItem(
            "challenge3Multi",
    ...
    const load = async (isImport = false) => {
        if (isImport) {
            ...
            if (saveFile) {
                try {
                    ...
                    alert(
                        "Save file invalid, using local save instead. Contact the developer on Discord if this is a mistake."
                    );
                }
            } else {
        ...
        if ($miningUpgradeLevels.length < 100) {
            $miningUpgradeLevels = [
                ...$miningUpgradeLevels,
                ...Array(100 - $miningUpgradeLevels.length).fill(0),
            ];
        }
        ...
        if (localStorage.getItem("miningUpgradeLevelsFree")) {
            miningUpgradeLevelsFree.set(
                JSON.parse(localStorage.getItem("miningUpgradeLevelsFree"))
            );
        }
        if (localStorage.getItem("miningUpgradeLevelsBoughtTemp")) {
            miningUpgradeLevelsBoughtTemp.set(
                JSON.parse(localStorage.getItem("miningUpgradeLevelsBought"))
            );
        }
        if (localStorage.getItem("miningUpgradeLevelsFreeTemp")) {
            miningUpgradeLevelsFreeTemp.set(
        ...
        $miningUpgradeLevels = $miningUpgradeLevels.map(
            (x, i) =>
                $miningUpgradeLevelsBought[i] + $miningUpgradeLevelsFree[i]
        );
        ...
        if (localStorage.getItem("keyItemsUnlocked")) {
            const loadedValues = JSON.parse(
                localStorage.getItem("keyItemsUnlocked")
            );
            const keyItems = {};
        ...
        if (localStorage.getItem("beaconSpendAmt")) {
            beaconSpendAmt.set(
                JSON.parse(localStorage.getItem("beaconSpendAmt"))
            );
        }
        $unlockedRes = new Set();
        ...
        if (localStorage.getItem("beaconProgress")) {
            beaconProgress.set(
                JSON.parse(localStorage.getItem("beaconProgress"))
            );
        }
        ...
        if (localStorage.getItem("beaconNextReqs")) {
            beaconNextReqs.set(
                JSON.parse(localStorage.getItem("beaconNextReqs"))
            );
        }
        if (localStorage.getItem("beaconUpgradeLevels")) {
            beaconUpgradeLevels.set(
                JSON.parse(localStorage.getItem("beaconUpgradeLevels"))
            );
        }
        ...
        if (localStorage.getItem("beaconActivations")) {
            beaconActivations.set(
                JSON.parse(localStorage.getItem("beaconActivations"))
            );
        }
        ...
        if (localStorage.getItem("buttonNumClicks")) {
            buttonNumClicks.set(
                JSON.parse(localStorage.getItem("buttonNumClicks"))
            );
        }
        if (localStorage.getItem("buttonUpgradeLevels")) {
            buttonUpgradeLevels.set(
                JSON.parse(localStorage.getItem("buttonUpgradeLevels"))
            );
        }
        ...
        if (localStorage.getItem("keyUpgradeLevels")) {
            keyUpgradeLevels.set(
                JSON.parse(localStorage.getItem("keyUpgradeLevels"))
            );
        }
        if (localStorage.getItem("keyCraftTimes")) {
            keyCraftTimes.set(
                JSON.parse(localStorage.getItem("keyCraftTimes"))
            );
        }
        if (localStorage.getItem("keyCraftAmount")) {
            keyCraftAmount.set(
                JSON.parse(localStorage.getItem("keyCraftAmount"))
            );
        }
        if (localStorage.getItem("keyCraftMastery")) {
            keyCraftMastery.set(
                JSON.parse(localStorage.getItem("keyCraftMastery"))
            );
        }
        ...
        if (localStorage.getItem("antiFlickerFlags")) {
            antiFlickerFlags.set(
                JSON.parse(localStorage.getItem("antiFlickerFlags"))
            );
        }
        ...
        if (localStorage.getItem("automationItemsUnlocked")) {
            automationItemsUnlocked.set(
                JSON.parse(localStorage.getItem("automationItemsUnlocked"))
            );
        }
        ...
        if (localStorage.getItem("activityLogShow")) {
            activityLogShow.set(
                JSON.parse(localStorage.getItem("activityLogShow"))
            );
        }
        if (localStorage.getItem("challengeActive")) {
            challengeActive.set(
                JSON.parse(localStorage.getItem("challengeActive"))
            );
        }
        if (localStorage.getItem("challengesCompleted")) {
            challengesCompleted.set(
                JSON.parse(localStorage.getItem("challengesCompleted"))
            );
        }
        if (localStorage.getItem("beaconSmartSplits")) {
            beaconSmartSplits.set(
                JSON.parse(localStorage.getItem("beaconSmartSplits"))
            );
        }
    ...
    function versionUpdater() {
            ...
            for (let x in $buttonUpgradeLevels) {
                if ($buttonUpgrades[x]) {
                    if (
                        ...
                        $buttonUpgradeLevels[x] =
                            $buttonUpgrades[x]["maxLevel"];
        ...
        if (ver < 14) {
            for (let i = 2; i < $enchantUpgrades.length; i++) {
                $activityLogShow[$enchantUpgrades[i]["name"].toLowerCase()] =
                    true;
            }
        ...
        if (ver < 17) {
            for (let i in $keysOpened) {
                if ($keysOpened[i] == Infinity || isNaN($keysOpened[i]))
                    $keysOpened[i] = 0;
            }

            // REMOVE THIS TO TEST AND DEBUG
            if (isNaN($wallet["key1"]) || $wallet["key1"] == Infinity)
                $wallet["key1"] = 0;
            if (isNaN($wallet["key2"]) || $wallet["key2"] == Infinity)
                $wallet["key2"] = 0;
            if (isNaN($wallet["key3"]) || $wallet["key3"] == Infinity)
                $wallet["key3"] = 0;
            if (isNaN($wallet["key4"]) || $wallet["key4"] == Infinity)
                $wallet["key4"] = 0;
            if (isNaN($wallet["key5"]) || $wallet["key5"] == Infinity)
                $wallet["key5"] = 0;

            for (let i = 0; i < 200; i++) {
                if (!$miningUpgradeLevelsBought[i])
        ...
        if (ver < 23) {
            $wallet["emagic"] = $wallet["goldenSigils"]
                ? $wallet["goldenSigils"]
                : undefined;
    ...
    const tabHelpText = {
        ...
        enchants:
            "Mine size and Mine quality affect many things, not just enchants! Make sure to check tooltips. \n When the enchant bar of each respective color fills, there's a chance for all enchants of the same color as the bar to fire (chance shown on tooltips), giving a very powerful bonus. Upgrade your enchants to increase the proc chance.",
        relocate:
            "Reset to gain fame. Fame can be spent on powerful mining upgrades and enchants. It's recommended for your first relocation to gain at least 25 fame, which should take about 1-2 hours. HINT: Are you stuck? Each fame multiplier is color-coded based on the resource it deals with. If you are struggling to increase fame gains, you may be able to increase a specific multiplier by checking the tab where that resource is generated/used.",
    ...
    onMount(() => {
        ...
        setInterval(() => {
            for (let tab of ref.tabs) {
                tabsUnlocked[tab] = !tabUnlockCriteria[tab]
                    ? false
