...
select-none'>
<span class='{$buttonUpgrades[index]['isRadiumUpgrade'] ? 'text-lime-300' : 'text-indigo-300'}'>
{$buttonUpgrades[index]['name']} [{f($buttonUpgradeLevels[index],0)} / {f($buttonUpgrades[index]['maxLevel'],0)}]
</span>
        ...
        <div class='text-center effect-wrapper'>
            <div class='tooltip-text-xs text-[#cccccc]'>
                 <span class='current text-[#cccccc]'>
                    {$buttonUpgrades[index]['prefix'] || ""}{$buttonUpgrades[index]['isPercent'] ?
                    fp($buttonUpgrades[index]['formula']($buttonUpgradeLevels[index]),3, false) :
                    f($buttonUpgrades[index]['formula']($buttonUpgradeLevels[index]),3)}{$buttonUpgrades[index]['suffix'] || ""}
                 </span>
                 <span class='current text-[#999999]'>  => 
                    {$buttonUpgrades[index]['prefix'] || ""}{$buttonUpgrades[index]['isPercent'] ?
                   fp($buttonUpgrades[index]['formula']($buttonUpgradeLevels[index]+$settings['buyAmount']),3, false) :
                   f($buttonUpgrades[index]['formula']($buttonUpgradeLevels[index]+$settings['buyAmount']),3)}{$buttonUpgrades[index]['suffix'] || ""}
                 </span>
...
// @ts-nocheck
    ...
    let getCosts = () => {
        let c = {};
        for (let [type, base] of Object.entries($buttonUpgrades[index]['cost'])) {
            c[type] = cost(base);
        }
        return c;
    }
    ...
    function cost(start) {
       const base = start * Math.pow($buttonUpgrades[index]['ratio'], $buttonUpgradeLevels[index]);  
    ...
    function isUnlocked() {
        for (let [type, val] of Object.entries($buttonUpgrades[index]['unlockAt'])) {
            if ($wallet[type] < val) {
                return false;
            }
        }  
