<script>
  ...
  function cost(start) {
    const base =
      start *
      Math.pow($buttonUpgrades[index]["ratio"], $buttonUpgradeLevels[index]);
    ...
    >
      <span
        class={$buttonUpgrades[index]["isRadiumUpgrade"]
          ...
          : "text-indigo-300"}
      >
        {$buttonUpgrades[index]["name"]} [{f($buttonUpgradeLevels[index], 0)} / {f(
          $buttonUpgrades[index]["maxLevel"],
          0
        )}]
      </span>
      <span
        ...
        <div class="text-center effect-wrapper">
          <div class="tooltip-text-xs text-[#cccccc]">
            <span class="current text-[#cccccc]">
              {$buttonUpgrades[index]["prefix"] || ""}{$buttonUpgrades[index][
                "isPercent"
              ]
                ? fp(
              ...
              {#if c[1] >= 1}
                <div
                  class="{ref.colors[c[0]] ||
                    ref.colors['default']} text-right pr-3 col-span-2"
                >
