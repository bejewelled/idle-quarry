<script>
  ...
  onMount(() => {
    ...
    const mainLoop = setInterval(() => {
      ...
      if ($automationItemsUnlocked["beaconizer"])
        $wallet["beacons"] =
          ($wallet["beacons"] || 0) + 50 * ((UPDATE_SPEED * dt) / 1000);
  ...
  function updateprogressThisTick(delta) {
    ...
    let progKey1 = Math.pow(
      ($miningUpgradeLevels[3] > 0
        ? PROGRESS_BASE * $miningUpgrades[3]["formula"]($miningUpgradeLevels[3])
        : 0) *
    ...
    let progKey2 = Math.pow(
      ($miningUpgradeLevels[4] > 0
        ? PROGRESS_BASE * $miningUpgrades[4]["formula"]($miningUpgradeLevels[4])
        : 0) *
        $miningUpgrades[26]["formula"]($miningUpgradeLevels[26]) *
        progressBonusMulti *
        $challenge3Multi *
  ...
  function addProgress(delta) {
    ...
    if (delta > 16 || document.hidden) {
      ...
      if ($enchantProgress["t1"] > $enchantThreshold["t1"]) {
        procEnchants(
          Math.floor($enchantProgress["t1"] / $enchantThreshold["t1"]),
          "t1"
        );
      ...
      if ($challengeActive !== 0) {
        for (let i of ["gems", "key1", "key2", "key3", "key4", "key5"]) {
          addChallengePoints(
            ($progressThisTick[i] || 0) / ($progressThreshold[i] || 1),
            i,
            true
          );
        }
  ...
  function addChallengePoints(n, t) {
    $wallet["challengePoints"] =
      ($wallet["challengePoints"] || 0) + formula.calcChallengePointGain(n, t);
  }
  ...
  function addGems(n, avgProgress) {
    ...
    const GEM_BASE = 1;
    const gemGain =
      (GEM_BASE + $miningUpgrades[1]["formula"]($miningUpgradeLevels[1])) *
      ...
      if ($visibleTier >= ref.dropTiers[item]) {
        if (vals[0] >= 1) {
          ...
          const numWins =
            vals[0] * n +
            Math.max(
              (Math.random() > 0.5 ? 1 : -1) * stdev * Math.pow(c / (c - 5), 6),
              0
            );
  ...
  function addBeaconProgress(delta, isOffFocus = false) {
    ...
    const challenge4Multi =
      $challengeActive == 4
        ? Math.max(1e-18, 1e-2 / Math.pow(10, $challengesCompleted[3] || 0))
        : 1;
        ...
        switch (i) {
          ...
          case 4: // lightning blast
            ...
            setTimeout(() => {
              progressBonusMulti /= Math.pow(size, 0.7) * 3 + 3;
              if (
                Math.abs(progressBonusMulti - Math.round(progressBonusMulti)) <
                0.1
              )
          ...
          case 6: // key boon
            ...
            if (keyRand > 1.18) {
              ...
              addToActivityLog(
                "[KEY BOON] INCREDIBLE! +" + f(reward) + " [*****] keys!",
                "text-amber-400",
                "key boon"
              );
              ...
              addToActivityLog(
                "[Key Boon] Excellent! +" + f(reward) + " [****] keys",
                "text-violet-400",
                "key boon"
              );
