<script>
    ...
    function updateprogressThisTick(delta) {
        ...
        let softcapThresh =
            $progressThreshold["gems"] * (UPDATE_SPEED / 1000) * 25;
        //softcap!
    ...
    function addProgress(delta) {
        ...
        if (
            delta > (1000 / UPDATE_SPEED) * 120 &&
            !$settings["offlineProgress"]
        )
            return;
    ...
    function procEnchants(n, tier) {
        ...
        const quality = $enchantUpgrades[1]["formula"](
            $enchantUpgradeLevels[1]
        );
